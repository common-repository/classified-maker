...
class class_classified_maker_shortcodes_post_ads_new{
	...
	public function post_ads($atts, $content = null ) {
			...
			if(isset($_POST['post_ads_hidden'])){
				...
				//$validations = $class_pickform->validations($cat_id, $_POST);
...
*/	
					...
					foreach($cat_fields as $key=>$field){
						...
						if(isset($field['checked']) && $field['checked'] == 'yes'){
							...
							if(!empty($meta_fields[$key])){
								...
								$html.= $class_pickform->field_set($meta_fields[$key]);
	...
	new class_classified_maker_shortcodes_post_ads_new();