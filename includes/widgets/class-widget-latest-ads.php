...
class WidgetLatestAds extends WP_Widget {

	function __construct() {
		
		parent::__construct(
			'classified_maker_widgets_latest_ads', 
			__('Classified Maker - Latest Ads', classified_maker_textdomain),
			array( 'description' => __( 'Show latest ads.', classified_maker_textdomain ), ) 
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		echo '<ul class="classified-maker-widgets latest-ads">';
		...
		if ( $wp_query->have_posts() ) :
			while ( $wp_query->have_posts() ) : $wp_query->the_post();	
				...
				if ( $show_image == 'on' ) {
					...
					echo '<div class="ads-image"><img  src="'.$thumb_url.'" /></div>';	
				}
		...
		echo '<style>
		.widgets_ads_details a { text-decoration:none;}
		li.widgets_single_ads {border-bottom: 1px solid #bbb;padding: 5px 0px;}
		li.widgets_single_ads:last-child {border-bottom: none;}
		.widgets_ads_details {padding: 0px 0px 10px;margin-left: 90px;}
		.widgets_ads_image {width: 80px;float:left;}
		</style>';
	...
	public function form( $instance ) {
		...
		if ( isset( $instance[ 'title' ] ) ) $title = $instance[ 'title' ];else $title = __( 'Latest Ads', classified_maker_textdomain );
		if ( isset( $instance[ 'count' ] ) ) $count = $instance[ 'count' ];else $count = __( '5', classified_maker_textdomain );
		if ( isset( $instance[ 'post_in' ] ) ) $post_in = $instance[ 'post_in' ];else $post_in = '';
