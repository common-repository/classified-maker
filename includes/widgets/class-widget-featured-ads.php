...
class WidgetFeaturedAds extends WP_Widget {

	function __construct() {
		
		parent::__construct(
			'classified_maker_widgets_featured_ads', 
			__('Classified Maker - Featured Ads', classified_maker_textdomain),
			array( 'description' => __( 'Show Featured Ads.', classified_maker_textdomain ), ) 
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$post_in 	= apply_filters( 'widget_title', $instance['post_in'] );
		...
		$wp_query = new WP_Query(
			array (
				...
				'meta_query' => array(
					array(
						'key'     => 'classified_maker_ads_featured',
						'value'   => 'yes',
						'compare' => 'LIKE',
					),
				),
		...
		echo '<ul class="classified-maker-widgets featured-ads">';
	...
	public function form( $instance ) {
		...
		else $title = __( 'Featured Ads', classified_maker_textdomain );
		
		if ( isset( $instance[ 'count' ] ) ) $count = $instance[ 'count' ];
		else $count = __( '5', classified_maker_textdomain );
		
		if ( isset( $instance[ 'post_in' ] ) ) $post_in = $instance[ 'post_in' ];
		else $post_in = '';
		
		if ( isset( $instance[ 'show_image' ] ) && $instance[ 'show_image' ] == 'on' ) $show_image = 'checked';
		else $show_image = '';
		
		if ( isset( $instance[ 'show_price' ] ) && $instance[ 'show_price' ] == 'on' ) $show_price = 'checked';
		else $show_price = '';
		
		if ( isset( $instance[ 'show_loc' ] ) && $instance[ 'show_loc' ] == 'on' ) $show_loc = 'checked';
		else $show_loc = '';
		
		if ( isset( $instance[ 'show_date' ] ) && $instance[ 'show_date' ] == 'on' ) $show_date = 'checked';
		else $show_date = '';
		...
		<li class="widgets_item_li">
			<ul>
				<li class="widgets_item_li_title"><label for="<?php echo $this->get_field_id( 'post_in' ); ?>"><?php _e( 'Show post by ID' ); ?></label> </li>
				<li class="widgets_item_li_input"><input class="" id="<?php echo $this->get_field_id( 'post_in' ); ?>" name="<?php echo $this->get_field_name( 'post_in' ); ?>" type="text" value="<?php echo esc_attr( $post_in ); ?>" placeholder="150,200"/></li>
			</ul>
		</li>
		<li class="widgets_item_li">
			<ul>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['post_in'] 	= ( ! empty( $new_instance['post_in'] ) ) ? strip_tags( $new_instance['post_in'] ) : '';
