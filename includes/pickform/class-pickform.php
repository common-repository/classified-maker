...
*/	
			...
			$field_html['select2'] = array(

									'html'=>$this->field_select2( $field_data ),														
...
*/
	...
	public function field_text_multi($field_data){
            ...
            if(!empty($field_values)){
                if(is_array($field_values)){
                    ...
                    foreach($field_values as $key=>$value){
						...
						$html.= '<input class="remove-field button" type="button" value="'.__('Remove',pickform_textdomain).'" />';
						$html.= '</div>';
            ...
            $html.= '</div>';
            $html.= '<input class="add-field button" option-id="'.$field_id.'" type="button" value="'.__('Add more',pickform_textdomain).'" />';
            $html.= '</div>';			
  ...
  $plupload_init = apply_filters('plupload_init', $plupload_init);
	...
	public function field_select2($field_data){
			...
			if( is_array( $input_values ) ) $value = $input_values[0];
			else $value = $input_values;
			
			if( empty( $value ) ) $html.= '<option value="">Type Company Name</option>';
			else $html.= '<option value="'.$value.'">'.get_the_title( $value ).'</option>';
